...
exports.createPump = async (req, res, next) => {
  ...
  try {
    ...
    if (existingPump) {
      if (isAutoGenerated) {
        // This should not happen with correct auto-generation
        console.error(`🔧 ERROR: Auto-generated pump number ${finalPumpNumber} already exists! This indicates a bug in auto-generation.`);
        await t.rollback();
        return res.status(500).json({ 
          success: false, 
          error: 'Internal error: Auto-generated pump number conflict. Please try again.' 
        });
      } else {
