...
exports.getSales = asyncHandler(async (req, res, next) => {
  ...
  return sendSuccess(res, filteredSales);
});
...
exports.getSalesSummary = asyncHandler(async (req, res, next) => {
  const { station_id, stationId, date, start_date, startDate, end_date, endDate } = req.query;
  const userId = req.user.id;
  const userRole = req.user.role;
  ...
  if (effectiveDate) {
    ...
    where.readingDate = { [Op.between]: [effectiveStartDate, effectiveEndDate] };
  } else if (effectiveStartDate) {
    where.readingDate = { [Op.gte]: effectiveStartDate };
  } else if (effectiveEndDate) {
    where.readingDate = { [Op.lte]: effectiveEndDate };
  }
  ...
  try {
    const txWhere = {};
    if (effectiveDate) txWhere.transactionDate = effectiveDate;
    if (effectiveStartDate && effectiveEndDate) txWhere.transactionDate = { [Op.between]: [effectiveStartDate, effectiveEndDate] };
    if (effectiveStartDate && !effectiveEndDate) txWhere.transactionDate = { [Op.gte]: effectiveStartDate };
    if (!effectiveStartDate && effectiveEndDate) txWhere.transactionDate = { [Op.lte]: effectiveEndDate };
    if (effectiveStationId) txWhere.stationId = effectiveStationId;
    ...
    logAudit({
      severity: 'debug',
      action: 'getSalesSummary_DailyTransaction_fetch_failed',
      details: e.message
    });
  }
