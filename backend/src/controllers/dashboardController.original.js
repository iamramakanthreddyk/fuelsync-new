 ...
 * 
 * Refactored to use service and repository layers
 */
...
exports.getNozzleBreakdown = async (req, res, next) => {
  try {
    ...
    const readings = await NozzleReading.findAll({
      where: {
        ...whereClause,
        transactionId: { [Op.ne]: null } // Ensure we only include readings with transactions
      },
      include: [{
    ...
    if (txnIds.length > 0) {
      const txns = await DailyTransaction.findAll({ where: { id: txnIds }, attributes: ['id', 'paymentBreakdown'], raw: true });
      for (const t of txns) txnCache[t.id] = t;
    }
...
exports.getDailySummary = async (req, res, next) => {
  try {
    ...
    // Bulk fetch transactions for this range to avoid N+1
    const allTxnIds = [...new Set(readings.map(r => r.transactionId).filter(Boolean))];
    let txnMap = {};
    if (allTxnIds.length > 0) {
      const txns = await DailyTransaction.findAll({ where: { id: allTxnIds }, attributes: ['id', 'paymentBreakdown'], raw: true });
      txnMap = txns.reduce((m, t) => { m[t.id] = t.paymentBreakdown || {}; return m; }, {});
    }
    for (const reading of readings) {
      ...
      if (reading.transactionId && !seenTransactions[date].has(reading.transactionId)) {
        ...
        const tb = txnMap[reading.transactionId] || null;
        if (tb) {
          dateMap[date].cash += parseFloat(tb.cash || 0);
          dateMap[date].online += parseFloat(tb.online || 0);
          dateMap[date].credit += parseFloat(tb.credit || 0);
        }
      }
    }
