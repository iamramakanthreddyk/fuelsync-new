 ...
 */
exports.getNozzleBreakdown = asyncHandler(async (req, res, next) => {
  const startTime = Date.now();
...
exports.getDailySummary = asyncHandler(async (req, res, next) => {
  ...
  if (!effectiveStartDate || !effectiveEndDate) {
    return sendError(res, 'MISSING_PARAMS', 'startDate and endDate are required', 400);
  }
 ...
 */
exports.getFuelBreakdown = asyncHandler(async (req, res, next) => {
  const startTime = Date.now();
  const { startDate, endDate, start_date, end_date, stationId } = req.query;
  const user = await User.findByPk(req.userId);
...
exports.getFinancialOverview = asyncHandler(async (req, res, next) => {
  ...
  const stationFilter = await dashboardRepo.getStationFilter(user, stationId);
  if (stationFilter === null) {
    return sendSuccess(res, { month: targetMonth, noData: true });
  }
 ...
 */
exports.getMissedEntriesAlert = asyncHandler(async (req, res, next) => {
 ...
 */
exports.getPendingHandoversAlert = asyncHandler(async (req, res, next) => {
  const user = await User.findByPk(req.userId);
  
  // Only return owner stats about their stations, employees, and sales
  return sendSuccess(res, { handovers: [] });
});
 ...
 */
exports.getShiftStatus = asyncHandler(async (req, res, next) => {
  const user = await User.findByPk(req.userId);
 ...
 */
exports.getOwnerStats = asyncHandler(async (req, res, next) => {
  const user = await User.findByPk(req.userId);
 ...
 */
exports.getIncomeReceivablesReport = asyncHandler(async (req, res, next) => {
  const { stationId, startDate, endDate } = req.query;

  if (!stationId) {
    return sendError(res, 'MISSING_FIELD', 'stationId required', 400);
  }
  ...
  // Aggregate payment breakdown from DailyTransaction
