...
export default function IncomeReport() {
  ...
  const totalExpenses = useMemo(() => {
    ...
    return expenses
      .filter((exp: any) => exp.approvalStatus === 'approved' || exp.approvalStatus === 'auto_approved')
      .reduce((sum: number, exp: any) => sum + parseSalesAmount(exp.amount), 0);
  }, [expenses]);

  const expensesByCategory = useMemo(() => {
    const categories: Record<string, number> = {};
    // Only count approved/auto-approved expenses
    expenses
      .filter((exp: any) => exp.approvalStatus === 'approved' || exp.approvalStatus === 'auto_approved')
      .forEach((exp: any) => {
        if (exp.category) {
