 ...
 */
export function toFixedNumber(val: any, decimals: number = 2, fallback: string = '0.00'): string {
  const n = Number(val);
 ...
 */
export function formatCurrency(val: any, currency: string = 'INR', decimals: number = 2): string {
  const n = Number(val);
